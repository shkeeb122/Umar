...
def generate_content(keyword, history=[]):
    messages = [
        {"role":"system",
         "content":"You are an ultra-intelligent AI marketing assistant. "
                  "Respond in human-like style with clear structure, headings, bullet points, spacing, and suggestions."}
    ] + history + [
...
def command():
    ...
    try:
        ...
        cursor.execute("INSERT INTO campaigns VALUES (?,?,?,?,?,?,?,?)",
                       (campaign_id, query, keyword, content, blog_url, "AI", json.dumps(conversation), summary, datetime.utcnow().isoformat()))
        conn.commit()
...
def chat(campaign_id):
    try:
        ...
        # Use last 20 messages + summary for context
        ...
        # Update DB + summary async for performance
        def update_db():
