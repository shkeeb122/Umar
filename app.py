...
cursor = conn.cursor()
cursor.execute("""CREATE TABLE IF NOT EXISTS campaigns(id TEXT PRIMARY KEY, niche TEXT, keywords TEXT, content TEXT, blog_url TEXT, source TEXT, created_at TEXT)""")
cursor.execute("""CREATE TABLE IF NOT EXISTS posts(id TEXT PRIMARY KEY, title TEXT, content TEXT, slug TEXT, created_at TEXT)""")
cursor.execute("""CREATE TABLE IF NOT EXISTS task_history(id TEXT PRIMARY KEY, campaign_id TEXT, step_name TEXT, status TEXT, source TEXT, note TEXT, timestamp TEXT)""")
conn.commit()
...
def serpapi_keywords(query):
    ...
    try:
        r = requests.get("https://serpapi.com/search.json", params={"engine":"google_autocomplete","q":query,"api_key":SERP_API_KEY}, timeout=10)
        r.raise_for_status()
        suggestions = [s["value"].strip() for s in r.json().get("suggestions", []) if s.get("value") and "2023" not in s["value"] and "2022" not in s["value"]]
...
def marketing_agent(command, conversation_history=[]):
    ...
    conn.commit()
    return {"status":"success","campaign_id":campaign_id,"niche":query,"keywords":keywords,"products":[{"name":k} for k in keywords],"blog_url":blog_url,"source":f"{source} (keywords), {content_source} (content)"}
