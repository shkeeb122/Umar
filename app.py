...
# DEPLOYED BACKEND URL (replace with your actual deployed URL)
...
id TEXT PRIMARY KEY, niche TEXT, keywords TEXT, products TEXT, content TEXT, blog_url TEXT, status TEXT, created_at TEXT)""")
...
def add_campaign_sql(niche, keywords, products, content, blog_url):
    ...
    cursor.execute("INSERT INTO campaigns VALUES (?,?,?,?,?,?,?,?)",
        (campaign_id, niche, ",".join(keywords), ",".join([p["name"] for p in products]),
         content, blog_url, "published", created_at))
    conn.commit()
...
def publish_local_blog(title, content):
    ...
    created_at = datetime.utcnow().isoformat()
    cursor.execute("INSERT INTO posts VALUES (?,?,?,?,?)",(post_id,title,content,slug,created_at))
    conn.commit()
...
def view_blog(slug):
    ...
    html = f"<html><head><title>{title}</title></head><body style='font-family:Arial;max-width:800px;margin:auto'><h1>{title}</h1><p><i>{created}</i></p><hr><div>{content}</div></body></html>"
...
def system_check():
    report = {}
    try:
        r = requests.post(MISTRAL_URL, headers=HEADERS, json={"model":MODEL_NAME,"messages":[{"role":"user","content":"test"}]}, timeout=30)
...
# HOME
@app.route("/")
def home(): return jsonify({"status":"AI Marketing System Running"})
...
# TOOLS
def niche_research_tool(): return ["AI software","fitness products","weight loss products","web hosting","online courses"]
def keyword_tool(niche): return [f"best {niche}",f"{niche} review",f"cheap {niche}",f"top {niche} 2026"]
def product_tool(niche): return [{"name":f"{niche} Pro Tool","commission":"40%"},{"name":f"{niche} Premium Kit","commission":"30%"}]
...
# CAMPAIGN HISTORY
@app.route("/campaigns")
def campaigns(): cursor.execute("SELECT * FROM campaigns"); return jsonify(cursor.fetchall())
