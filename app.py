...
def content_tool(keyword, conversation_history=[]):
    ...
    try:
        ...
        fallback_content = f"This is fallback content for '{keyword}' (API failed)."
...
def command_route():
    ...
    return jsonify(marketing_agent(cmd, history))

@app.route("/campaigns")
def get_campaigns():
    cursor.execute("SELECT id, niche, created_at FROM campaigns ORDER BY created_at DESC")
    campaigns = cursor.fetchall()
    result = [{"id": c[0], "niche": c[1], "created_at": c[2]} for c in campaigns]
    return jsonify({"status":"success","campaigns": result})
